# -*- coding: utf-8 -*-
"""
Created on Fri Nov  1 13:09:15 2019
...
for i in range(len(master_pattern_info['A1'])):
    p300s.distribute(67, deck_labware['1'].wells('A1'), deck_labware['2'].wells(master_pattern_info['A1'][i]), new_tip='always')
  
for i in range(len(master_pattern_info['A2'])):
    p300s.distribute(67, deck_labware['1'].wells('B2'), deck_labware['2'].wells(master_pattern_info['A2'][i]), new_tip='always')
...
for i in range (8):
    for j in range (6):
        p10m.transfer (10, deck_labware['2'].wells(master_pattern_info['A1'][i]), 
						  deck_labware['3'].wells(master_pattern_info['A1'][i][0]+final_pattern_info['1'][j]), new_tip='never')
        p10m.blow_out()
p10m.drop_tip()        
...
for i in range (8):
	for j in range (6):
            p10m.transfer (10, deck_labware['2'].wells(master_pattern_info['A2'][i]), 
						  deck_labware['3'].wells(master_pattern_info['A2'][i][0]+final_pattern_info['2'][j]), new_tip='never')
            p10m.blow_out()
p10m.drop_tip() 
